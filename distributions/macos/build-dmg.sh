#!/bin/bash
# ============================================================================
# Trexo PDF Signer - macOS DMG Builder
...
#
# Output:
#   ./output/trexo-pdf-signer-x64-macos.dmg
# ============================================================================
...
# Configuration
DMG_NAME="trexo-pdf-signer-x64-macos"
VOLUME_NAME="TrexoPDFSigner"
...
# App path
APP_PATH="$OUTPUT_DIR/TrexoPDFSigner.app"

echo "============================================================================"
echo " Building Trexo PDF Signer macOS DMG Installer"
echo "============================================================================"
...
if [ ! -d "$APP_PATH" ]; then
    echo "ERROR: TrexoPDFSigner.app not found at $APP_PATH"
    echo ""
...
# Copy app to staging
echo "  Copying TrexoPDFSigner.app..."
...
cat > "$DMG_DIR/Install Trexo PDF Signer.command" << INSTALLSCRIPT
#!/bin/bash
# ============================================================================
# Trexo PDF Signer - macOS Installation Script with Version Checking
# TrexoLab - https://trexolab.com
# ============================================================================
...
echo ""
echo -e "\${BLUE}============================================================================\${NC}"
echo -e "\${BLUE} Trexo PDF Signer Installation Script - Version \$NEW_VERSION\${NC}"
echo -e "\${BLUE}============================================================================\${NC}"
echo ""
...
install_app() {
    ...
    if is_app_running; then
        echo -e "\${RED}  ERROR: Trexo PDF Signer is currently running.\${NC}"
        echo "  Please close the application before proceeding with the installation."
        exit 1
    fi
...
# Main installation
echo "This script will install Trexo PDF Signer to your Applications folder."
echo "It will check for existing versions and handle upgrades safely."
echo ""
echo "App to install:"
echo "  - TrexoPDFSigner.app (4GB max memory)"
echo ""
...
# Install app
install_app "TrexoPDFSigner.app"
...
echo -e "\${GREEN}============================================================================\${NC}"
echo ""
echo " You can now launch Trexo PDF Signer from your Applications folder."
echo ""
...
INSTALLSCRIPT
chmod +x "$DMG_DIR/Install Trexo PDF Signer.command"

# Create uninstaller script
cat > "$DMG_DIR/Uninstall Trexo PDF Signer.command" << UNINSTALLSCRIPT
#!/bin/bash
# ============================================================================
# Trexo PDF Signer - macOS Uninstallation Script
# TrexoLab - https://trexolab.com
# ============================================================================

APPLICATIONS_DIR="/Applications"
APP_NAME="TrexoPDFSigner.app"
APP_PATH="\$APPLICATIONS_DIR/\$APP_NAME"
CONFIG_DIR="\$HOME/.trexo-pdf-signer"
...
clear
echo -e "\${BLUE}============================================================================\${NC}"
echo -e "\${BLUE} Trexo PDF Signer Uninstaller\${NC}"
echo -e "\${BLUE}============================================================================\${NC}"
echo ""

if [ ! -d "\$APP_PATH" ]; then
    echo -e "\${RED}Error: Trexo PDF Signer is not found in /Applications.\${NC}"
    echo "If you installed it elsewhere, please delete it manually."
    exit 1
fi
...
if pgrep -f "TrexoPDFSigner.jar" > /dev/null; then
    echo -e "\${RED}ERROR: Trexo PDF Signer is currently running.\${NC}"
    echo "Please close the application before proceeding with uninstallation."
    exit 1
fi

read -p "Are you sure you want to uninstall Trexo PDF Signer? [y/N] " -n 1 -r
echo ""
if [[ ! \$REPLY =~ ^[Yy]\$ ]]; then
...
if [ -d "\$CONFIG_DIR" ]; then
    echo ""
    echo "Found user configuration at: \$CONFIG_DIR"
    echo "This includes signature profiles and preferences."
    read -p "Do you want to remove user configuration as well? [y/N] " -n 1 -r
    echo ""
    if [[ \$REPLY =~ ^[Yy]\$ ]]; then
        echo "Removing \$CONFIG_DIR..."
        rm -rf "\$CONFIG_DIR"
    else
        echo "Keeping user configuration."
    fi
fi
...
read -p "Press Enter to close this window..."
UNINSTALLSCRIPT
chmod +x "$DMG_DIR/Uninstall Trexo PDF Signer.command"

# Create README
cat > "$DMG_DIR/README.txt" << EOF
============================================================================
 Trexo PDF Signer $APP_VERSION - Installation Instructions
============================================================================

This DMG contains Trexo PDF Signer with 4GB maximum memory allocation.

INSTALLATION:

Option 1: Using the Install Script (Recommended)
   Double-click "Install Trexo PDF Signer.command" to run the installer.
   This will:
...
Option 2: Manual Drag and Drop
   1. Drag TrexoPDFSigner.app to the "Applications" folder
   2. Eject this disk image
   3. Open Trexo PDF Signer from your Applications folder

   On first launch:
   Right-click the app → Select "Open" → Click "Open" in the dialog

IMPORTANT:
   - Do NOT run Trexo PDF Signer directly from this disk image
...
- Website: https://trexolab-solution.github.io/trexo-pdf-signer/
...
tell application "Finder"
    tell disk "$VOLUME_NAME"
        ...
        set position of item "TrexoPDFSigner.app" of container window to {150, 150}
        set position of item "Applications" of container window to {450, 150}
        -- Position install script and README below
        set position of item "Install Trexo PDF Signer.command" of container window to {200, 350}
...
echo " Contents:"
echo "   - TrexoPDFSigner.app (4GB max memory)"
echo "   - Install Trexo PDF Signer.command (installer with version checking)"
...
echo ""
echo " To install:"
echo "   Option 1: Double-click 'Install Trexo PDF Signer.command' (recommended)"
echo "             - Checks for existing versions"
echo "             - Handles upgrades automatically"
echo "             - Prevents accidental downgrades"
echo ""
echo "   Option 2: Drag TrexoPDFSigner.app to Applications folder"
echo "============================================================================"