; ============================================================================
; Trexo PDF Signer - Windows x64 Installer Script
...
;   iscc distributions\windows\trexo-pdf-signer-x64.iss
;
; ============================================================================
...
; #define MyAppAssocName "Trexo PDF Signer"
...
; Per-machine: C:\Program Files\TrexoLab\TrexoPDFSigner
DefaultDirName={autopf}\TrexoLab\TrexoPDFSigner
...
OutputDir=Output
OutputBaseFilename=trexo-pdf-signer-x64-setup
...
; Launcher scripts
Source: "run-trexo-pdf-signer.bat"; DestDir: "{app}"; Flags: ignoreversion; Components: main
...
//********** Try to find if app is running by checking for lock file or process
...
begin
  ...
  // Method 1: Check for .lock file in user data directory
  LockFile := GetUserHomeDir + '\.trexo-pdf-signer\.lock';
  if FileExists(LockFile) then
  begin
    Log('Lock file found at: ' + LockFile);
    Result := True;
    Exit;
  end;

  // Method 2: Check for process (using AppId as mutex is also possible,
  // but pgrep-like check is more reliable for Java apps)
  // Inno Setup's AppId often creates a mutex automatically if configured,
  // but we'll add a manual check here if needed.
end;
...
begin
  ...
  begin
    MsgBox('{#MyAppName} is currently running.' + #13#10#13#10 +
           'Please close the application and any open PDF files before continuing with the installation.',
           mbError, MB_OK);
    Result := False;
    Exit;
  end;
...
begin
  ...
  begin
    Log('Trexo PDF Signer installation completed successfully.');
...
begin
  ...
  begin
    Log('Uninstalling Trexo PDF Signer...');
  end;
