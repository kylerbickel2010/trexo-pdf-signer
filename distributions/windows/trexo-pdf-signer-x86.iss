; ============================================================================
; Trexo PDF Signer - Windows x86 (32-bit) Installer Script
; Inno Setup 6.x Script
; TrexoLab - https://trexolab.com
; ============================================================================
;
; This installer packages:
; - TrexoPDFSigner.jar (the main application)
; - Bundled JRE 8 x86 (no system Java required)
...
;   iscc distributions\windows\trexo-pdf-signer-x86.iss
;
; ============================================================================

; NOTE: Different AppID from x64 to allow parallel installation
#define MyAppID "7A13BBC1-965E-4184-A7F3-589BDFEB1A49"
#define MyAppName "Trexo PDF Signer"
...
#define MyAppURL "https://github.com/trexolab-solution/trexo-pdf-signer"
#define MyAppPublisherURL "https://trexolab.com"
#define MyAppExeName "run-trexo-pdf-signer.bat"
#define MyAppLauncher "trexo-pdf-signer-launcher.vbs"
#define MyAppCopyright "Copyright (C) 2024-2025 TrexoLab"

[Setup]
; Unique application ID - DO NOT change after release (different from x64)
AppId={#MyAppID}
...
; Per-user: C:\Users\<user>\AppData\Local\Programs\TrexoLab\TrexoPDFSigner
; Per-machine: C:\Program Files (x86)\TrexoLab\TrexoPDFSigner
DefaultDirName={autopf32}\TrexoLab\TrexoPDFSigner
DefaultGroupName={#MyAppName}
DisableProgramGroupPage=yes

; Installer output settings
OutputDir=Output
OutputBaseFilename=trexo-pdf-signer-x86-setup
SetupIconFile=logo.ico
UninstallDisplayIcon={app}\logo.ico

; Branding images
WizardImageFile=trexo-pdf-signer-inno.bmp
...
; Architecture settings - 32-bit only
ArchitecturesAllowed=x86 x64compatible
; Do NOT set ArchitecturesInstallIn64BitMode - we want 32-bit mode
...
VersionInfoDescription={#MyAppName} Setup (32-bit)
...
[Messages]
WelcomeLabel1=Welcome to {#MyAppName} Setup (32-bit)
WelcomeLabel2=This will install {#MyAppName} {#MyAppVersion} (32-bit) on your computer.%n%n{#MyAppName} includes a bundled Java 8 runtime - no separate Java installation is required.%n%nIt is recommended that you close all other applications before continuing.
...
[Components]
Name: "main"; Description: "Trexo PDF Signer Application"; Types: full compact custom; Flags: fixed
Name: "jre"; Description: "Bundled Java 8 Runtime (x86)"; Types: full custom; Flags: fixed
...
Source: "..\..\target\TrexoPDFSigner.jar"; DestDir: "{app}"; Flags: ignoreversion; Components: main

; Launcher scripts (use x86 version)
Source: "run-trexo-pdf-signer-x86.bat"; DestDir: "{app}"; DestName: "run-trexo-pdf-signer.bat"; Flags: ignoreversion; Components: main
Source: "trexo-pdf-signer-launcher.vbs"; DestDir: "{app}"; Flags: ignoreversion; Components: main

; Bundled JRE 8 x86
Source: "jre8-x86\*"; DestDir: "{app}\jre8-x86"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: jre
...
Name: "{group}\{#MyAppName}"; Filename: "wscript.exe"; Parameters: """{app}\{#MyAppLauncher}"""; WorkingDir: "{app}"; IconFilename: "{app}\logo.ico"; Comment: "Launch Trexo PDF Signer (Normal - 2GB)"

; Start Menu - Large profile
Name: "{group}\{#MyAppName} (Large)"; Filename: "wscript.exe"; Parameters: """{app}\{#MyAppLauncher}"" large"; WorkingDir: "{app}"; IconFilename: "{app}\logo.ico"; Comment: "Launch Trexo PDF Signer (Large - 4GB)"

; Start Menu - Extra Large profile
Name: "{group}\{#MyAppName} (Extra Large)"; Filename: "wscript.exe"; Parameters: """{app}\{#MyAppLauncher}"" xlarge"; WorkingDir: "{app}"; IconFilename: "{app}\logo.ico"; Comment: "Launch Trexo PDF Signer (Extra Large - 8GB)"

; Start Menu - Uninstall shortcut
Name: "{group}\Uninstall {#MyAppName}"; Filename: "{uninstallexe}"; Comment: "Uninstall Trexo PDF Signer"
...
begin
  ...
  begin
    Log('Trexo PDF Signer (32-bit) installation completed successfully.');
...
var
  ...
  UserDataPath: String;
begin
  UserDataPath := GetUserHomeDir + '\.trexo-pdf-signer';

  // Only show option if user data exists
  if DirExists(UserDataPath) or DirExists(ExpandConstant('{localappdata}\.trexo-pdf-signer')) then
  begin
...
begin
  ...
  begin
    Log('Uninstalling Trexo PDF Signer (32-bit)...');
  end;
  ...
  begin
    ...
    begin
      ...
      // Remove user data from home directory (~/.trexo-pdf-signer)
      UserDataPath := GetUserHomeDir + '\.trexo-pdf-signer';
      DelTreeSafe(UserDataPath);

      // Remove from LocalAppData
      LocalAppDataPath := ExpandConstant('{localappdata}\.trexo-pdf-signer');
      DelTreeSafe(LocalAppDataPath);

      // Remove from AppData/Roaming
      UserAppDataPath := ExpandConstant('{userappdata}\.trexo-pdf-signer');
...
begin
  ...
  Result := Result + Space + GetUserHomeDir + '\.trexo-pdf-signer' + NewLine;
end;
