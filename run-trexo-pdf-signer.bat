...
REM   run-trexo-pdf-signer.bat large     # Large profile (requires 64-bit Java 8)
REM   run-trexo-pdf-signer.bat xlarge    # Extra Large profile (requires 64-bit Java 8)
REM ============================================================================
...
if %ERRORLEVEL% neq 0 (
    ...
    echo.
    echo Trexo PDF Signer requires Java 8 ^(1.8.x^). Please install from:
...
if not "%JAVA_MAJOR%"=="8" (
    echo WARNING: Java %JAVA_VERSION% detected. Trexo PDF Signer requires Java 8 ^(1.8.x^).
    echo.
...
set JAR_FILE=
for %%f in (target\TrexoPDFSigner*.jar) do set JAR_FILE=%%f

if not defined JAR_FILE (
    REM Try current directory
    for %%f in (TrexoPDFSigner*.jar) do set JAR_FILE=%%f
)
...
echo ============================================================================
echo  Trexo PDF Signer - Advanced PDF Signing Application
echo ============================================================================
...
if %ERRORLEVEL% neq 0 (
    ...
    if %ERRORLEVEL%==1 (
        ...
        echo      run-trexo-pdf-signer.bat large
        echo      run-trexo-pdf-signer.bat xlarge
    )
    pause
)
