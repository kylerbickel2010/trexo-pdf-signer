 ...
 * Profiles are stored as JSON in ~/.trexo-pdf-signer/appearance-profiles.json
 */
public class AppearanceProfileManager {
    ...
    private AppearanceProfileManager() {
        this.profilesPath = AppConstants.PROFILES_PATH;
        ...
        try {
            Files.createDirectories(AppConstants.CONFIG_DIR_PATH);
        } catch (IOException e) {
            log.error("Failed to create " + AppConstants.CONFIG_DIR_NAME + " directory", e);
        }
