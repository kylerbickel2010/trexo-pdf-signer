...
import com.trexolab.utils.ReleaseNoteParser;
...
/**
 * Handles version checking for Trexo PDF Signer.
...
public class VersionManager {
    ...
    /**
     * Fetches release notes as pre-rendered HTML from GitHub API.
     * Uses GitHub's HTML rendering - no external markdown library needed.
     *
     * @param version Version tag (e.g., "V1.1.2")
     * @return Release notes as HTML or null if unable to fetch
     */
    public static String getReleaseNotes(String version) {
        ...
        try {
            ...
            conn.setRequestMethod("GET");
            // Request HTML rendering from GitHub API
            conn.setRequestProperty("Accept", "application/vnd.github.html+json");
            conn.setConnectTimeout(TIMEOUT_MS);
            conn.setReadTimeout(TIMEOUT_MS);
            conn.connect();
            ...
            // Extract body_html field (pre-rendered HTML from GitHub)
            String jsonResponse = response.toString();
            String bodyHtml = extractJsonField(jsonResponse, "body_html");

            if (bodyHtml != null && !bodyHtml.trim().isEmpty()) {
                // Unescape HTML entities that were JSON-escaped
                bodyHtml = unescapeJsonString(bodyHtml);
                return bodyHtml.trim();
            }

            // Fallback to plain body if body_html not available
     ...
     * Features a larger, resizable window with properly expanding release notes area.
     *
     * @param parent        Parent component for the dialog
    ...
    public static void showUpdateDialog(Component parent, String latestVersion) {
        ...
        JLabel titleLabel = new JLabel("<html><div style='text-align:center;'>"
                + "<p style='font-size:18pt; font-weight:bold; color:#4CAF50; margin:0;'>"
                + "\u2728 New Version Available!</p>"
                + "<p style='font-size:11pt; color:#AAAAAA; margin-top:10px;'>"
                + "A newer version of " + AppConstants.APP_NAME_SHORT + " is ready for download</p>"
                + "</div></html>");
        titleLabel.setHorizontalAlignment(SwingConstants.CENTER);
        headerPanel.add(titleLabel, BorderLayout.CENTER);

        // ===== VERSION INFO PANEL =====
        JPanel versionInfoWrapper = new JPanel(new BorderLayout());
        versionInfoWrapper.setBackground(new Color(45, 45, 45));
        versionInfoWrapper.setBorder(BorderFactory.createEmptyBorder(15, 25, 10, 25));

        JPanel versionPanel = new JPanel(new GridLayout(1, 4, 15, 0));
        ...
        currentLabel.setForeground(new Color(150, 150, 150));
        ...
        currentValueLabel.setForeground(new Color(200, 200, 200));
        currentValueLabel.setFont(new Font("SansSerif", Font.BOLD, 13));

        JLabel latestLabel = new JLabel("Latest Version:");
        latestLabel.setForeground(new Color(150, 150, 150));
        ...
        latestValueLabel.setFont(new Font("SansSerif", Font.BOLD, 13));
        ...
        versionInfoWrapper.add(versionPanel, BorderLayout.CENTER);

        // ===== RELEASE NOTES PANEL (CENTER - EXPANDABLE) =====
        JPanel changelogPanel = new JPanel(new BorderLayout(0, 10));
        changelogPanel.setBackground(new Color(45, 45, 45));
        changelogPanel.setBorder(BorderFactory.createEmptyBorder(10, 25, 15, 25));

        JLabel changelogTitle = new JLabel("Release Notes:");
        changelogTitle.setForeground(new Color(220, 220, 220));
        changelogTitle.setFont(new Font("SansSerif", Font.BOLD, 14));
        ...
        if (releaseNotes != null && !releaseNotes.trim().isEmpty()) {
            ...
            String htmlContent = ReleaseNoteParser.markdownToHtml(releaseNotes);

            JEditorPane changelogPane = new JEditorPane();
            changelogPane.setContentType("text/html");
            changelogPane.setText(htmlContent);
            changelogPane.setEditable(false);
            changelogPane.setCaretPosition(0);
            changelogPane.setBackground(new Color(35, 35, 35));
            ...
            JPanel noNotesPanel = new JPanel(new GridBagLayout());
            noNotesPanel.setBackground(new Color(35, 35, 35));
            noNotesPanel.setBorder(BorderFactory.createLineBorder(new Color(60, 60, 60), 1));

            JLabel noNotesLabel = new JLabel("<html><div style='text-align:center; padding:30px;'>"
                    + "<p style='font-size:12pt; color:#888888;'>Release notes not available.</p>"
                    + "<p style='font-size:10pt; color:#666666; margin-top:8px;'>Visit GitHub for more details.</p>"
                    + "</div></html>");
            noNotesLabel.setHorizontalAlignment(SwingConstants.CENTER);
            noNotesPanel.add(noNotesLabel);

            changelogPanel.add(noNotesPanel, BorderLayout.CENTER);
        }
        ...
        laterButton.setPreferredSize(new Dimension(140, 36));
        laterButton.setFocusPainted(false);
        laterButton.setFont(new Font("SansSerif", Font.PLAIN, 12));

        JButton downloadButton = new JButton("Download Now");
        downloadButton.setPreferredSize(new Dimension(150, 36));
        ...
        // ===== ASSEMBLE TOP SECTION (Header + Version Info) =====
        JPanel topSection = new JPanel(new BorderLayout());
        topSection.setBackground(new Color(45, 45, 45));
        topSection.add(headerPanel, BorderLayout.NORTH);
        topSection.add(versionInfoWrapper, BorderLayout.SOUTH);

        // ===== MAIN CONTENT PANEL =====
        JPanel mainPanel = new JPanel(new BorderLayout());
        mainPanel.setBackground(new Color(45, 45, 45));
        mainPanel.add(topSection, BorderLayout.NORTH);
        mainPanel.add(changelogPanel, BorderLayout.CENTER);  // CENTER allows expansion
        mainPanel.add(buttonPanel, BorderLayout.SOUTH);

        // Assemble dialog
        dialog.add(mainPanel);

        // Set preferred and minimum size for better display
    ...
    public static void showUpToDateDialog(Component parent) {
        ...
        JLabel messageLabel = new JLabel("<html><div style='width:300px; text-align:center;'>"
                ...
                + "You are running the latest version of <b>" + AppConstants.APP_NAME_SHORT + "</b>.</p>"
