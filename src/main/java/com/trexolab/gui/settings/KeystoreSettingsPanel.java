...
public class KeystoreSettingsPanel extends JPanel {
    ...
    private void browseAndAddPath(Consumer<String> addPathMethod) {
        ...
        if (AppConstants.isLinux) {
            extension = "so";
            chooser.setFileFilter(new FileNameExtensionFilter("PKCS#11 Library (*.so)", extension));
        } else if (AppConstants.isMac) {
            extension = "dylib";
            chooser.setFileFilter(new FileNameExtensionFilter("PKCS#11 Library (*.dylib)", extension));
        } else {
            extension = "dll";
            chooser.setFileFilter(new FileNameExtensionFilter("PKCS#11 Library (*.dll)", extension));
        }

        if (chooser.showOpenDialog(parentDialog) == JFileChooser.APPROVE_OPTION) {

            File selectedFile = chooser.getSelectedFile();
            if(!selectedFile.exists() || !selectedFile.isFile() || !selectedFile.getName().toLowerCase().endsWith("." + extension)) {
                DialogUtils.showError(
                        parentDialog,
                        "Invalid File Selection",
                        "Please select a valid PKCS#11 library file (*." + extension + ")."
                );
                return;
            }
