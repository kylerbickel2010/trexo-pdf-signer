        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground mb-8">
              Documents signed with Trexo PDF Signer are fully compatible with Adobe Reader DC and other
            ...
            <ul className="space-y-4">
              {compatibilityItems.map((item, index) => (
                <li
                  key={item}
                  className={`flex items-center gap-3 transition-all duration-500 ${contentVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
                    }`}
          ...
          <div
            ref={visualRef}
            className={`relative transition-all duration-700 ${visualVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
              }`}
          >
            <div className="group relative p-4 rounded-2xl glass-card border border-border/60 hover:border-primary/30 transition-all duration-500 ease-out">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-xl bg-red-500/10 flex items-center justify-center ring-1 ring-white/5 transition-all duration-500 group-hover:scale-105 group-hover:ring-white/10">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground group-hover:text-foreground/80 transition-colors duration-500">Industry standard PDF signatures</p>
                </div>
              </div>
              <LazyImage
                src={`${import.meta.env.BASE_URL}images/signed-pdf-view.png`}
                alt="Signed PDF in Adobe Reader"
                className="w-full h-auto rounded-lg object-contain bg-background/50"
              />
            ...
            {/* Floating badge */}
            <div
              className={`absolute top-2 right-2 px-4 py-2 rounded-xl glass-card border border-border/50 animate-float transition-all duration-700 z-10 ${visualVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
                }`}
