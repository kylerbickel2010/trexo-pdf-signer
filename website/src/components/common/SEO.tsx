...
export function SEO({
  ...
  const fullTitle = title ? `${title} | Trexo PDF Signer` : DEFAULT_TITLE;
  ...
  return (
    <Helmet>
      ...
      <meta property="og:site_name" content="Trexo PDF Signer" />
...
export const structuredDataGenerators = {
  softwareApplication: () => ({
    ...
    description: 'Trexo PDF Signer is a free, open-source PDF signing software and modern alternative to jSignPDF and Adobe Reader DC. Sign PDF documents with USB DSC tokens, PKCS#11 hardware tokens, and PKCS#12/PFX certificates. Creates legally valid digital signatures with green tick checkmark verification.',
    url: BASE_URL,
    downloadUrl: DOWNLOAD_URLS.windowsInstaller,
    softwareVersion: '1.0',
  ...
  imageGallery: (images: { url: string; title: string; description: string }[]) => ({
    '@context': 'https://schema.org',
    '@type': 'ImageGallery',
    name: 'Trexo PDF Signer Screenshots',
    description: 'Gallery of Trexo PDF Signer application screenshots showing PDF signing features',
