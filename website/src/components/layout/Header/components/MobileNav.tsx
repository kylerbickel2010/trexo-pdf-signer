...
import { GITHUB_URL, NAV_LINKS, PAGE_LINKS, MORE_LINKS } from '@/utils/constants';
...
// Combine NAV_LINKS and MORE_LINKS for mobile view (flattened structure)
...
export function MobileNav({
  ...
  return (
    <>
      ...
      <div
        className={`lg:hidden fixed inset-0 top-16 bg-black/70 backdrop-blur-md z-40 transition-all duration-400 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        onClick={onClose}
        aria-hidden="true"
      />
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden fixed top-16 left-0 right-0 z-50 transition-all duration-400 ease-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'
          }`}
      >
                ...
                {SECTION_LINKS.map((link, index) => {
                  ...
                  return link.isSection ? (
                    <button
                      ...
                      className={`w-full px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 text-left cursor-pointer flex items-center gap-3 group ${active
                    ...
                    <Link
                      ...
                      to={link.href}
                      onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 cursor-pointer group ${active
                        ? 'text-primary bg-primary/10 shadow-sm shadow-primary/5'
                        : 'text-muted-foreground hover:text-foreground hover:bg-white/5 active:bg-white/10'
                        }`}
                      style={getAnimationStyle(index + 2)}
