...
import { GITHUB_URL, NAV_LINKS, MORE_LINKS } from '@/utils/constants';
    ...
    if (link.isSection) {
      return (
        <button
          ...
          className={`px-3 py-1.5 text-sm font-medium transition-colors cursor-pointer-custom ${active ? 'text-primary' : 'text-muted-foreground hover:text-foreground'
            }`}
        >
          {link.name}
        </button>
      ...
      <nav className="flex items-center bg-white/5 rounded-full px-1 py-1 border border-white/10">
        ...
        <div ref={moreRef} className="relative">
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 text-sm font-medium transition-colors cursor-pointer-custom ${SECONDARY_NAV.items.some(item => isActive(item, activeSection))
                ? 'text-primary'
            ...
            <div className="absolute top-full right-0 mt-2 py-1.5 min-w-[140px] rounded-xl bg-slate-900/95 backdrop-blur-xl border border-white/10 shadow-xl animate-fade-in">
              {SECONDARY_NAV.items.map((link) => (
                <button
                  ...
                  className={`w-full px-4 py-2 text-sm text-left transition-colors ${isActive(link, activeSection)
